...
# 'alpha' is the overall regularization strength (lambda)
# 'l1_ratio' is the mixing parameter (r)
lambda_val = 0.1
alpha = lambda_val 
l1_ratio_val = 0.5 # 50% L1 (Lasso) and 50% L2 (Ridge) penalty

print(f"\nTraining Elastic Net model with alpha (lambda) = {alpha} and l1_ratio = {l1_ratio_val}...")

# Initialize and train the ElasticNet model
# KEY CHANGE: Using ElasticNet
...
elastic_net_model.fit(X_train, y_train_flat)
...
# Calculate metrics
rmse_en = np.sqrt(mean_squared_error(y_test_flat, y_pred_en))
mae_en = mean_absolute_error(y_test_flat, y_pred_en)

print("\n--- Scikit-learn ELASTIC NET Results ---")
print(f"Alpha (lambda): {alpha}")
print(f"L1 Ratio (r): {l1_ratio_val}")
